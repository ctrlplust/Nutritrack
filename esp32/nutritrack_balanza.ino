...
#define SERVER_BASE      "https://nqiht-201-188-79-192.run.pinggy-free.link/api/nutritrack"
// ───────────────────────────────────────────
  ...
  if (httpCode > 0) {
    if (httpCode == 200) {
      ...
      if (!error) {
        ...
        if (doc.containsKey("activeProduct") && !doc["activeProduct"].isNull()) productoDetectado = true;
        else if (doc.containsKey("id") || doc.containsKey("name") || doc.containsKey("barcode")) productoDetectado = true;
        else if (payload.indexOf("id") != -1 || payload.indexOf("product") != -1) productoDetectado = true;
...
void enviarConsumo(float consumidoG, float pesoFinalG, float pesoInicialG) {
  ...
  if (httpCode > 0) {
    ...
    if (httpCode == 200 || httpCode == 201) Serial.println("✅ ¡Consumo registrado en la app!");
    else Serial.println("⚠ El servidor devolvió error. Revisa que la app tenga un producto activo.");
  } else {
